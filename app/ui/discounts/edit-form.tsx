...
import { updateDiscountAction, State } from '@/app/lib/actions';
...
export default function EditDiscountForm({ discount }: { discount: Discount }) {
  const initialState: State = { message: null, errors: {} };
  const updateWithId = updateDiscountAction.bind(null, discount.id);
  const [state, formAction] = useActionState(updateWithId, initialState);
  const [discountType, setDiscountType] = useState<'percentage' | 'fixed'>(discount.discount_type);

  // Convert values from Kobo to Naira for display
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                type="number"
                step="0.01"
                defaultValue={displayMinPurchase}
                placeholder="0"
                className="peer block w-full rounded-xl border border-slate-200 py-2.5 pl-10 text-sm text-slate-900 outline-none placeholder:text-slate-400 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500"
              />
              <BanknotesIcon className="pointer-events-none absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" />
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                name="expires_at"
                type="date"
                defaultValue={formatDateForInput(discount.expires_at)}
                className="peer block w-full rounded-xl border border-slate-200 py-2.5 pl-10 text-sm text-slate-900 outline-none placeholder:text-slate-400 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500"
              />
