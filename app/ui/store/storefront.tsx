...
function useButtonProps(theme: StoreTheme) {
  ...
  const style = (() => {
    switch (theme.button_style) {
      ...
      case "soft":
        return {
          backgroundColor: `${theme.primary_color}18`,
          color: theme.primary_color,
          border: "none",
        };
      case "glass":
        return {
          backgroundColor: `${theme.surface_color || "#fff"}cc`,
  ...
  const hover = (() => {
    switch (theme.animation_style) {
      ...
      case "bounce":
        return "hover:-translate-y-1.5 hover:scale-[1.03] active:scale-95";
      case "fade":
        return "hover:opacity-80 active:opacity-60";
      default:
  ...
  return {
    ...
    style,
    hover,
    className: `transition-all duration-300 ${radiusClass} ${hover} focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2`,
  };
}
...
export default function Storefront({
  ...
  const [deliveryLocation, setDeliveryLocation] = useState<{
    ...
    customer?.delivery_latitude && customer?.delivery_longitude
      ? {
          lat: customer.delivery_latitude,
          lng: customer.delivery_longitude,
          details: customer.delivery_address_details || "",
        }
      : null,
  );
  ...
  const borderRadiusClass = getBorderRadiusClass(
    activeTheme.border_radius as any,
  );
  const spacingClass =
  ...
  const headerClass =
    activeTheme.header_style === "static"
      ...
      : activeTheme.header_style === "transparent"
        ? "absolute top-0 left-0 right-0 z-[100] bg-transparent"
  ...
  const headerStyles = {
    ...
    borderBottomWidth:
      activeTheme.header_style === "transparent"
        ...
        : activeTheme.card_style === "bold"
          ? "3.5px"
          : activeTheme.card_style === "minimal"
            ? "0px"
            : "1px",
    boxShadow:
      activeTheme.header_style === "transparent"
        ? "none"
        : getCardShadowClass(activeTheme.card_shadow || "soft"),
    color:
      activeTheme.header_style === "transparent"
        ? "#ffffff"
  ...
  useEffect(() => {
    ...
    fonts.forEach((f) => {
      if (f && f !== "inter") {
        const family =
          fontFamilyMap[f] || f.charAt(0).toUpperCase() + f.slice(1);
        const id = `gfont-${f}`;
  ...
  const headerIconColor = isTransparentHeader
    ...
    : activeTheme.text_color;
  const headerIconBg = isTransparentHeader
    ? "rgba(255,255,255,0.12)"
    : `${activeTheme.primary_color}0d`;
  const headerIconHoverBg = isTransparentHeader
    ? "rgba(255,255,255,0.2)"
    : `${activeTheme.primary_color}18`;
  const boldBorder =
    activeTheme.card_style === "bold"
  ...
  const cartButton = (
    <div className="flex items-center gap-1">
      ...
      <button
        ...
        onMouseEnter={(e) => {
          e.currentTarget.style.backgroundColor = headerIconHoverBg;
        }}
        onMouseLeave={(e) => {
          e.currentTarget.style.backgroundColor = headerIconBg;
        }}
  ...
  const logoOrInitial = !(
    activeTheme.show_logo ?? true
  ) ? null : activeTheme.logo_url ? (
            ...
            : logoFrame === "rounded" || logoFrame === "minimal"
              ? activeTheme.border_radius === "pill"
                ...
                : activeTheme.border_radius === "sharp"
                  ? "0"
                  : "0.5rem"
              : logoFrame === "none" || logoFrame === "plain"
                ? "0.375rem"
                : "50%",
      }}
    >
      ...
      <h1
        ...
        style={{
          color:
            activeTheme.header_style === "transparent"
              ? "#ffffff"
              : activeTheme.heading_color || activeTheme.text_color,
        ...
        {placedOrder.paymentMethod === "cash" && vendor.bank_name && (
          <div className="mt-6 w-full max-w-md rounded-2xl border-2 border-emerald-200 bg-emerald-50 p-6">
            ...
            <p className="mt-4 text-xs text-emerald-700">
              Please transfer the exact amount and notify the vendor via
              WhatsApp
            </p>
          </div>
        )}
  ...
  const renderProductGrid = () => {
    ...
    const cardShadowClass = getCardShadowClass(
      activeTheme.card_shadow || "soft",
    );
    const cardHoverEffect = getCardHoverEffect(
      activeTheme.card_style || "modern",
    );
      ...
      >
        <div className="mb-6 flex items-center justify-between">
          <h3
            ...
            style={{
              color: activeTheme.heading_color || activeTheme.text_color,
              fontFamily: FONT_MAP[activeTheme.heading_font] || undefined,
            }}
          >
            Products
        ...
        <div
          className={`grid ${spacingClass} ${
            activeTheme.layout_style === "grid"
              ? activeTheme.layout_width === "standard"
                ? "grid-cols-1 sm:grid-cols-2"
                : "grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4"
              : activeTheme.layout_style === "list"
                ? "grid-cols-1"
                : activeTheme.layout_width === "standard"
                  ? "grid-cols-1 sm:grid-cols-2"
                  : "grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4"
          ...
          {activeProducts.length === 0 ? (
            <div className="col-span-1 sm:col-span-2 py-16 px-6 text-center rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50">
              ...
              <p className="text-sm text-slate-500 max-w-sm mx-auto">
                This store looks amazing, but the vendor is still adding their
                products to the shelves. Please check back later!
              </p>
            </div>
          ) : (
              ...
              return (
                ...
                >
                  {activeTheme.show_product_images && (
                    <div
                      className={`${imageAspectClass} relative flex items-center justify-center bg-slate-50 text-slate-200 overflow-hidden`}
                    >
                      {product.image_url ? (
                        <Image
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    <h4
                      ...
                      style={{
                        color:
                          activeTheme.heading_color || activeTheme.text_color,
                        fontFamily:
                          FONT_MAP[activeTheme.heading_font] || undefined,
                      }}
                    >
                      {product.name}
                    </h4>
                    ...
                    <div className="mt-4 flex items-end justify-between pt-4 border-t border-slate-50">
                      ...
                      <button
                        ...
                        style={{
                          ...btn.style,
                          padding: hasOptions ? "0.5rem 1rem" : "",
                          height: "2.5rem",
                          width: hasOptions ? "auto" : "2.5rem",
                          fontSize: "0.875rem",
                          ["--tw-ring-color" as any]: activeTheme.primary_color,
                        }}
                      >
      ...
      {activeTheme.custom_css && (
        <style
          dangerouslySetInnerHTML={{
            __html: activeTheme.custom_css.replace(/<\/style>/gi, ""),
          }}
        />
      )}
        ...
        style={
          {
            ...
            fontFamily:
              FONT_MAP[activeTheme.font_family] || activeTheme.font_family,
          } as React.CSSProperties
        }
      >
        <DynamicNav
        ...
        <main
          className={`mx-auto px-4 pb-32 ${layoutWidthClass} ${
            [
              ...
              "midnight-luxe",
              "vogue-minimal",
            ].includes(activeTheme.template_id)
              ? "pt-0"
              : activeTheme.template_id === "tech-store"
                ? "pt-8"
                : activeTheme.header_style === "transparent" &&
                    sections[0]?.id !== "hero-banner"
                  ? "pt-24"
                  : "pt-8"
          }`}
        >
        ...
        <footer className={`mx-auto px-4 pb-24 pt-8 ${layoutWidthClass}`}>
          <div
        ...
        {cartCount > 0 &&
          !isCartOpen &&
          !activeTheme.show_mobile_checkout_bar && (
            <footer
              className={`fixed bottom-0 left-1/2 -translate-x-1/2 w-full p-4 z-10 ${layoutWidthClass}`}
            >
              <button
                onClick={() => setIsCartOpen(true)}
              ...
              <div className="w-screen max-w-md transform bg-white shadow-2xl transition-transform duration-300 ease-in-out">
                <div className="flex h-full flex-col">
                  <div className="flex items-center justify-between border-b border-slate-100 px-6 py-6 font-bold text-slate-900">
                    ...
                    <button
                      onClick={() => setIsCartOpen(false)}
                      className="rounded-full p-2 hover:bg-slate-50 text-slate-400 hover:text-slate-600 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400"
                    >
                      <StoreIcon
                        name="close"
                        theme={activeTheme}
                        ...
                        <div className="pt-8 border-t border-slate-100">
                          {!isCheckingOut && !appliedDiscount && (
                            <div className="mb-6 flex flex-col gap-2">
                              ...
                              {discountError && (
                                <p className="text-xs font-medium text-red-600 px-2">
                                  {discountError}
                                </p>
                              )}
                            </div>
                              ...
                              <div className="flex items-center justify-between text-sm font-bold text-emerald-600 bg-emerald-50 px-3 py-2.5 rounded-xl border border-emerald-100">
                                <div className="flex items-center gap-2">
                                  ...
                                  <button
                                    ...
                                    className="rounded-full bg-emerald-200 text-emerald-700 hover:bg-emerald-300 w-5 h-5 flex items-center justify-center text-sm transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
