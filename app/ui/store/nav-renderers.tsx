...
// ─── Shared Logo Block ───
function LogoBlock({ vendor, theme, scrolled = false, isTransparent = false }: { vendor: User; theme: StoreTheme; scrolled?: boolean; isTransparent?: boolean }) {
  ...
  const borderRadiusClass = borderRadius === 'sharp' ? 'rounded-none' : borderRadius === 'pill' ? 'rounded-full' : 'rounded-xl';
  ...
  return (
    <div className={`flex w-full ${logoPos === 'center' ? 'flex-col items-center justify-center text-center gap-2' : 'items-center gap-3'}`}>
      {logoContent}
      {nameBlock}
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className={`mx-auto px-6 md:px-10 ${layoutWidthClass}`}>
        <div className={`items-center gap-4 ${theme.logo_position === 'center' ? 'grid grid-cols-3 min-h-[6rem] py-4' : 'flex justify-between h-14 md:h-20'}`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : (theme.logo_position === 'right' ? 'order-2' : '')}`}>
            {theme.logo_position === 'center' ? (
              <button onClick={handleShare} className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-50 transition-colors hover:bg-slate-100" style={{ color: theme.text_color }}>
          ...
          <div className={`flex items-center gap-2 md:gap-4 ${theme.logo_position === 'center' ? 'justify-end' : (theme.logo_position === 'left' ? 'order-2' : '')}`}>
            {theme.logo_position !== 'center' && (
              <button onClick={handleShare} className="text-sm font-medium hover:opacity-75 transition-opacity px-2" style={{ color: scrolled ? theme.text_color : '#ffffff', fontFamily: FONT_MAP['dmSans'] }}>Spread</button>
            )}
            <button onClick={() => setIsCartOpen(true)} className="flex h-10 px-4 items-center justify-center gap-2 rounded-xl text-sm font-medium transition-colors border shadow-sm" style={{ backgroundColor: scrolled ? theme.surface_color : 'rgba(255,255,255,0.1)', color: scrolled ? theme.primary_color : '#ffffff', borderColor: scrolled ? theme.border_color : 'transparent' }}>
              <StoreIcon name="cart" theme={theme} className="w-4 h-4" />
              <span>Cart</span>
              {cartCount > 0 && <span className="ml-1 bg-green-800 text-white rounded-full px-2 py-0.5 text-[10px] shadow-sm">{cartCount}</span>}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className={`mx-auto px-4 md:px-8 ${layoutWidthClass}`}>
        <div className={`items-center gap-4 py-2 ${theme.logo_position === 'center' ? 'grid grid-cols-3 min-h-[5rem] md:min-h-[6rem]' : 'flex justify-between h-14 md:h-16'}`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : 'max-w-[50%]'}`}>
            {theme.logo_position === 'center' ? (
              <button 
                type="button" 
                onClick={handleShare} 
                className="flex h-10 w-10 items-center justify-center rounded-lg border border-[rgba(148,163,184,0.15)] bg-slate-800/10 hover:bg-slate-800/20" 
                style={{ color: '#94a3b8' }}
              >
                <StoreIcon name="share" theme={theme} className="w-5 h-5" />
              </button>
          ...
          <div className={`flex shrink-0 items-center gap-4 ${theme.logo_position === 'center' ? 'justify-end' : ''}`}>
            {theme.logo_position === 'center' ? (
              <button type="button" onClick={() => setIsCartOpen(true)} className="relative flex items-center justify-center h-10 w-10 rounded-lg group overflow-hidden" style={{ color: '#ffffff', background: `linear-gradient(135deg, ${theme.primary_color}, ${theme.secondary_color})`, boxShadow: cartCount > 0 ? `0 0 20px ${theme.primary_color}40` : 'none' }}>
                <StoreIcon name="cart" theme={theme} className="w-5 h-5 z-10 transition-transform group-hover:scale-110" />
                <div className="absolute inset-0 bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity" />
                {cartCount > 0 && <span className="absolute -top-1 -right-1 flex h-5 min-w-[1.25rem] items-center justify-center rounded-full px-1 text-[10px] font-black z-20 shadow-lg" style={{ backgroundColor: theme.accent_color, color: '#0a0f1c' }}>{cartCount}</span>}
              </button>
            ) : (
              <>
  ...
  return (
    ...
    >
      <div className={`mx-auto px-4 sm:px-6 md:px-8 py-4 ${layoutWidthClass}`}>
        <div className={`items-center gap-4 ${theme.logo_position === 'center' ? 'grid grid-cols-3 min-h-[6rem] py-4' : 'flex justify-between h-14 md:h-20'}`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : (theme.logo_position === 'right' ? 'order-last' : '')}`}>
            {theme.logo_position === 'center' ? (
              ...
              <LogoBlock vendor={vendor} theme={theme} isTransparent={isTransparentHeader} />
            )}
          </div>
          ...
          <div className={`flex items-center gap-3 ${theme.logo_position === 'center' ? 'justify-end' : ''}`}>
            {theme.logo_position !== 'center' && (
              <button onClick={handleShare} className="flex h-10 w-10 items-center justify-center rounded-full hover:bg-black/5" style={{ color: headerIconColor }}>
                <StoreIcon name="share" theme={theme} className="w-5 h-5" />
              </button>
            )}
            <button onClick={() => setIsCartOpen(true)} className="relative flex h-10 px-4 items-center gap-2 rounded-full font-bold text-sm tracking-wide transition transform hover:scale-105 active:scale-95 shadow-sm" style={{ backgroundColor: theme.primary_color, color: '#fff' }}>
...
export function BeautyGlowNav({ vendor, theme, cartCount, setIsCartOpen, handleShare, layoutWidthClass }: NavProps) {
  ...
  return (
    ...
    >
      <div className={`mx-auto px-6 md:px-10 ${layoutWidthClass}`}>
        <div className={`items-center gap-4 ${theme.logo_position === 'center' ? 'grid grid-cols-3 min-h-[6.5rem] py-4' : 'flex justify-between h-16 md:h-20'}`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : (theme.logo_position === 'right' ? 'order-2' : '')}`}>
             {theme.logo_position === 'center' ? (
                <button onClick={handleShare} className="flex h-10 w-10 items-center justify-center rounded-full bg-pink-50 text-pink-500 hover:bg-pink-100 transition-colors">
                  <StoreIcon name="share" theme={theme} className="w-5 h-5" />
                </button>
             ) : (
                <LogoBlock vendor={vendor} theme={theme} scrolled={scrolled} isTransparent />
          ...
          <div className="flex items-center justify-end gap-3 md:gap-5">
             {theme.logo_position !== 'center' && (
                <button onClick={handleShare} className="text-sm font-semibold hover:opacity-70 transition-opacity" style={{ color: scrolled ? theme.primary_color : '#ffffff' }}>Share</button>
             ) }
             <button onClick={() => setIsCartOpen(true)} className="relative flex h-10 px-5 items-center justify-center gap-2 rounded-full text-white font-bold transition-all hover:shadow-lg hover:translate-y-[-1px]" style={{ backgroundColor: theme.primary_color, backgroundImage: `linear-gradient(135deg, ${theme.primary_color}, ${theme.secondary_color})` }}>
               <StoreIcon name="cart" theme={theme} className="w-4 h-4" />
               <span className="text-xs uppercase tracking-widest">Cart</span>
               {cartCount > 0 && <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-white text-[10px] font-black shadow-sm" style={{ color: theme.primary_color }}>{cartCount}</span>}
             </button>
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      <div className={`mx-auto px-6 md:px-12 ${layoutWidthClass}`}>
        <div className={`items-center gap-6 ${theme.logo_position === 'center' ? 'grid grid-cols-3 min-h-[7rem] py-6' : 'flex justify-between h-16 md:h-24'}`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : (theme.logo_position === 'right' ? 'order-last' : 'flex-1')}`}>
             {theme.logo_position === 'center' ? (
                <button type="button" onClick={handleShare} className="group flex h-11 w-11 items-center justify-center rounded-full bg-white/5 border border-white/10 transition-all duration-300 hover:bg-white/10" style={{ color: '#e2e0ea' }}>
                  <StoreIcon name="share" theme={theme} className="w-5 h-5" />
                </button>
             ) : (
                <LogoBlock vendor={vendor} theme={theme} scrolled={scrolled} isTransparent />
          ...
          <div className={`flex items-center gap-4 ${theme.logo_position === 'center' ? 'justify-end' : ''}`}>
             {theme.logo_position !== 'center' && (
            ...
            <button type="button" onClick={() => setIsCartOpen(true)} className="group relative flex h-11 px-5 items-center gap-3 rounded-full transition-all duration-300 hover:bg-white/10 shadow-lg" style={{ color: '#e2e0ea', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)' }}>
              <span className="hidden md:block text-[10px] font-bold tracking-[0.2em] uppercase">Cart</span>
  ...
  return (
    ...
    >
      <div className="w-full px-4 md:px-8">
        <div className={`items-center gap-4 ${theme.logo_position === 'center' ? 'grid grid-cols-3 min-h-[6rem] py-4' : 'flex justify-between h-16 md:h-20'}`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : (theme.logo_position === 'right' ? 'order-2 flex gap-4' : '')}`}>
             {theme.logo_position === 'center' ? (
                <button type="button" onClick={handleShare} className="group flex h-10 w-10 items-center justify-start transition-all hover:opacity-50" style={{ color: '#000000' }}>
                  <StoreIcon name="share" theme={theme} className="w-5 h-5 transition-transform group-hover:scale-110" />
                </button>
             ) : (
                <LogoBlock vendor={vendor} theme={theme} scrolled={scrolled} />
          ...
          <div className={`flex items-center gap-4 ${theme.logo_position === 'center' ? 'justify-end' : (theme.logo_position === 'left' ? 'flex items-center' : '')}`}>
            {theme.logo_position !== 'center' && (
              <button type="button" onClick={handleShare} className="group flex h-10 w-10 items-center justify-center transition-all hover:opacity-50" style={{ color: '#000000' }}>
                <StoreIcon name="share" theme={theme} className="w-4 h-4 md:h-5 md:w-5 transition-transform group-hover:scale-110" />
              </button>
            )}
            <button type="button" onClick={() => setIsCartOpen(true)} className="group relative flex h-10 items-center justify-end transition-all hover:opacity-50" style={{ color: '#000000' }}>
...
export function FreshMarketNav({ vendor, theme, cartCount, setIsCartOpen, handleShare, layoutWidthClass }: NavProps) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[100] transition-all duration-300" style={{ backgroundColor: scrolled ? 'rgba(255,255,255,0.95)' : theme.surface_color || '#ffffff', backdropFilter: scrolled ? 'blur(10px)' : 'none', borderBottom: `1px solid ${theme.border_color}`, borderTop: `4px solid ${theme.primary_color}`, boxShadow: scrolled ? '0 4px 20px rgba(0,0,0,0.04)' : 'none' }}>
      <div className={`mx-auto px-4 md:px-8 ${layoutWidthClass}`}>
        <div className={`items-center gap-4 ${theme.logo_position === 'center' ? 'grid grid-cols-3 min-h-[7rem] md:min-h-[8rem] py-4' : 'flex justify-between h-16 md:h-18'}`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : (theme.logo_position === 'right' ? 'order-2' : 'flex-1')}`}>
             {theme.logo_position === 'center' ? (
                <button onClick={handleShare} className="flex h-12 w-12 items-center justify-center rounded-full bg-slate-50 border border-slate-100 shadow-sm hover:scale-110 transition-transform" style={{ color: theme.primary_color }}>
          ...
          <div className={`flex items-center justify-end gap-4 ${theme.logo_position === 'center' ? '' : 'flex-1'}`}>
            {theme.logo_position !== 'center' && (
            ...
            <button onClick={() => setIsCartOpen(true)} className="relative flex h-12 px-6 items-center gap-2 rounded-full font-bold transition transform hover:translate-y--1 shadow-md hover:shadow-lg" style={{ backgroundColor: theme.primary_color, color: '#fff' }}>
              <StoreIcon name="cart" theme={theme} className="w-5 h-5" />
              <span className="hidden sm:inline">Basket</span>
              {cartCount > 0 && <span className="absolute -top-1 -right-1 flex h-6 min-w-[24px] items-center justify-center rounded-full bg-yellow-400 px-1 text-[12px] font-black text-green-900 border-2 border-white shadow-sm">{cartCount}</span>}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <header className="sticky top-0 z-[100]" style={{ backgroundColor: theme.surface_color, borderBottom: `3px solid ${theme.text_color}` }}>
      <div className={`mx-auto px-4 md:px-6 ${layoutWidthClass}`}>
        <div className={`items-center gap-4 ${theme.logo_position === 'center' ? 'grid grid-cols-3 py-6 min-h-[6.5rem]' : 'flex justify-between h-14 md:h-18'}`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : (theme.logo_position === 'right' ? 'order-2' : '')}`}>
             {theme.logo_position === 'center' ? (
                <button onClick={handleShare} className="flex h-10 w-10 items-center justify-center rounded border border-white/10 bg-white/5 hover:bg-white/10 transition-colors" style={{ color: theme.primary_color }}>
                  <StoreIcon name="share" theme={theme} className="w-5 h-5" />
                </button>
             ) : (
          ...
          <div className="flex items-center gap-4 justify-end">
            {theme.logo_position !== 'center' && (
               <button onClick={handleShare} className="flex h-10 w-10 items-center justify-center rounded border border-white/10 bg-white/5 text-slate-400 hover:text-white" style={{ borderColor: theme.border_color }}>
                 <StoreIcon name="share" theme={theme} className="w-4 h-4" />
               </button>
            )}
            <button onClick={() => setIsCartOpen(true)} className="group relative flex h-10 px-6 items-center justify-center gap-2 rounded font-black italic uppercase tracking-tighter transition-all hover:scale-105 active:scale-95" style={{ backgroundColor: theme.primary_color, color: '#000', boxShadow: cartCount > 0 ? `0 0 20px ${theme.primary_color}50` : 'none' }}>
              <StoreIcon name="cart" theme={theme} className="w-4 h-4 transition-transform group-hover:rotate-12" />
              <span>Loot</span>
              {cartCount > 0 && <span className="absolute -top-2 -right-2 flex h-5 min-w-[20px] items-center justify-center rounded-full bg-white text-[10px] font-black text-black ring-2 ring-red-600">{cartCount}</span>}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <header className="relative z-[100]" style={{ backgroundColor: theme.surface_color, borderBottom: `1px solid ${theme.border_color}` }}>
      <div className={`mx-auto px-4 md:px-8 py-6 ${layoutWidthClass}`}>
        ...
        <div className={`items-center gap-6 ${theme.logo_position === 'center' ? 'grid grid-cols-3 min-h-[6.5rem] py-4' : 'flex justify-between h-16 md:h-18'}`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : (theme.logo_position === 'right' ? 'order-2' : '')}`}>
             {theme.logo_position === 'center' ? (
                <button onClick={handleShare} className="flex h-11 w-11 items-center justify-center rounded-full border border-amber-100 bg-amber-50/30 hover:bg-amber-50 transition-colors" style={{ color: theme.text_color }}>
                  <StoreIcon name="share" theme={theme} className="w-5 h-5" />
                </button>
             ) : (
                <LogoBlock vendor={vendor} theme={theme} />
          ...
          <div className={`flex items-center gap-4 ${theme.logo_position === 'center' ? 'justify-end' : ''}`}>
             {theme.logo_position !== 'center' && (
                <button onClick={handleShare} className="flex items-center justify-center w-10 h-10 rounded-full border border-transparent hover:border-amber-200" style={{ color: theme.text_color }}>
                  <StoreIcon name="share" theme={theme} className="w-4 h-4" />
                </button>
             )}
             <button onClick={() => setIsCartOpen(true)} className="group flex items-center gap-3 text-sm font-medium transition-all" style={{ color: theme.text_color }}>
                <span className="hidden md:inline-block opacity-70 group-hover:opacity-100">Bag ({cartCount})</span>
                <span className="relative flex h-11 w-11 items-center justify-center rounded-full border transition-colors group-hover:bg-amber-900 group-hover:text-white" style={{ borderColor: theme.border_color, backgroundColor: cartCount > 0 ? theme.primary_color : 'transparent', color: cartCount > 0 ? '#fff' : theme.text_color }}>
                  <StoreIcon name="cart" theme={theme} className="w-5 h-5" />
                  {cartCount > 0 && <span className="absolute -top-1 -right-1 flex h-4 min-w-[16px] items-center justify-center rounded-full text-[9px] font-bold bg-white shadow-sm" style={{ color: theme.primary_color }}>{cartCount}</span>}
                </span>
             </button>
          </div>
        </div>
    ...
    <header className="fixed top-0 left-0 right-0 z-[100] transition-colors duration-300" style={{ backgroundColor: scrolled ? 'rgba(255,255,255,0.98)' : 'transparent', borderBottom: scrolled ? '1px solid #f0f0f0' : '1px solid transparent' }}>
      <div className={`mx-auto px-6 md:px-12 ${layoutWidthClass}`}>
        <div className={`items-center gap-4 ${theme.logo_position === 'center' ? 'grid grid-cols-3 min-h-[6.5rem] py-4' : 'flex justify-between h-16 md:h-20'}`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : (theme.logo_position === 'right' ? 'order-2' : '')}`}>
             {theme.logo_position === 'center' ? (
                <button onClick={handleShare} className="text-xs tracking-widest uppercase font-semibold hover:opacity-50 transition-opacity px-2" style={{ color: scrolled ? '#000' : '#fff' }}>Share</button>
             ) : (
                <LogoBlock vendor={vendor} theme={theme} scrolled={scrolled} />
             )}
          </div>
          ...
          <div className={`flex items-center gap-6 ${theme.logo_position === 'center' ? 'justify-end' : ''}`}>
             {theme.logo_position !== 'center' && (
                <button onClick={handleShare} className="text-xs tracking-widest uppercase font-semibold hover:opacity-50 transition-opacity" style={{ color: scrolled ? '#000' : '#fff' }}>Share</button>
             )}
            <button onClick={() => setIsCartOpen(true)} className="flex items-center gap-2 text-xs tracking-widest uppercase font-bold hover:opacity-50 transition-opacity" style={{ color: scrolled ? '#000' : '#fff' }}>
              Cart {cartCount > 0 && <span className="ml-1 opacity-60">({cartCount})</span>}
            </button>
          </div>
        </div>
      </div>
...
export function ArtisanDarkNav({ vendor, theme, cartCount, setIsCartOpen, handleShare, layoutWidthClass }: NavProps) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[100] transition-all duration-500" style={{ backgroundColor: scrolled ? 'rgba(28, 25, 23, 0.9)' : theme.surface_color, backdropFilter: scrolled ? 'blur(12px)' : 'none', borderBottom: scrolled ? `1px solid ${theme.border_color}` : '1px solid transparent' }}>
      <div className={`mx-auto px-6 md:px-10 ${layoutWidthClass}`}>
        <div className={`items-center gap-6 ${theme.logo_position === 'center' ? 'grid grid-cols-3 min-h-[6rem] py-4' : 'flex justify-between h-16 md:h-20'}`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : (theme.logo_position === 'right' ? 'order-2' : '')}`}>
             {theme.logo_position === 'center' ? (
                <button onClick={handleShare} className="text-[10px] uppercase tracking-[0.3em] hover:opacity-70 transition-opacity px-3 py-2 border border-white/10" style={{ color: theme.text_color, fontFamily: FONT_MAP['poppins'] }}>Spread</button>
             ) : (
                <LogoBlock vendor={vendor} theme={theme} scrolled={scrolled} />
             )}
          </div>
          ...
          <div className={`flex items-center gap-5 ${theme.logo_position === 'center' ? 'justify-end' : ''}`}>
            {theme.logo_position !== 'center' && (
              <button onClick={handleShare} className="text-[10px] uppercase tracking-[0.3em] hover:opacity-70 transition-opacity" style={{ color: theme.text_color, fontFamily: FONT_MAP['poppins'] }}>Spread</button>
            )}
            <button onClick={() => setIsCartOpen(true)} className="relative flex h-10 px-6 items-center justify-center rounded-full transition-all duration-300 hover:bg-neutral-800" style={{ backgroundColor: theme.primary_color, color: '#f5f5f5', border: `1px solid ${theme.border_color}` }}>
              <span className="text-[10px] font-bold uppercase tracking-[0.2em]" style={{ fontFamily: FONT_MAP['poppins'] }}>Cart {cartCount > 0 && `(${cartCount})`}</span>
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <header className="sticky top-0 z-[100]" style={{ backgroundColor: theme.surface_color, borderBottom: `1px solid ${theme.primary_color}` }}>
      ...
      <div className={`mx-auto px-6 md:px-8 relative z-10 ${layoutWidthClass}`}>
        <div className={`items-center gap-6 ${theme.logo_position === 'center' ? 'grid grid-cols-3 py-6' : 'flex justify-between h-14 md:h-16'}`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : (theme.logo_position === 'right' ? 'order-2' : '')}`}>
             {theme.logo_position === 'center' ? (
                <button onClick={handleShare} className="flex h-10 w-10 items-center justify-center border border-[rgba(0,255,255,0.2)] bg-cyan-500/5 hover:bg-cyan-500/20 shadow-[0_0_10px_rgba(0,255,255,0.1)] transition-all" style={{ color: theme.primary_color }}>
                  <StoreIcon name="share" theme={theme} className="w-5 h-5" />
                </button>
             ) : (
                <LogoBlock vendor={vendor} theme={theme} />
          ...
          <div className={`flex items-center gap-4 justify-end ${theme.logo_position === 'center' ? '' : ''}`}>
            {theme.logo_position !== 'center' && (
            ...
            <button onClick={() => setIsCartOpen(true)} className="relative flex h-10 px-6 items-center justify-center gap-3 border uppercase tracking-widest text-xs font-bold transition-all hover:translate-y-[-2px] hover:shadow-[0_0_20px_rgba(0,255,255,0.3)]" style={{ backgroundColor: `${theme.primary_color}10`, borderColor: theme.primary_color, color: theme.heading_color, boxShadow: `inset 0 0 15px ${theme.primary_color}30` }}>
              <span className="hidden sm:inline">Access Registry</span>
              <StoreIcon name="cart" theme={theme} className="w-5 h-5" />
              {cartCount > 0 && <span className="absolute -top-2 -right-2 flex h-5 min-w-[20px] items-center justify-center text-[10px] font-black shadow-[0_0_10px_#00ffff]" style={{ backgroundColor: theme.accent_color, color: '#000', border: `1px solid ${theme.primary_color}` }}>{cartCount}</span>}
            </button>
          </div>
        </div>
      </div>
...
export function MinimalSaaSNav({ vendor, theme, cartCount, setIsCartOpen, handleShare, layoutWidthClass }: NavProps) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[100] transition-all duration-300" style={{ backgroundColor: scrolled ? 'rgba(255,255,255,0.85)' : theme.surface_color || '#ffffff', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', borderBottom: scrolled ? `1px solid ${theme.border_color}` : '1px solid transparent' }}>
      <div className={`mx-auto px-6 md:px-12 ${layoutWidthClass}`}>
        <div className={`items-center gap-8 ${theme.logo_position === 'center' ? 'grid grid-cols-3 py-6' : 'flex justify-between h-16'}`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : (theme.logo_position === 'right' ? 'order-2' : '')}`}>
             {theme.logo_position === 'center' ? (
                <button onClick={handleShare} className="text-sm font-semibold tracking-wide hover:text-slate-400 transition-colors uppercase" style={{ color: theme.text_color, fontFamily: FONT_MAP['inter'] }}>Share</button>
             ) : (
                <LogoBlock vendor={vendor} theme={theme} scrolled={scrolled} />
             )}
          </div>
          ...
          <div className="flex items-center gap-6 justify-end">
            ...
            <button onClick={() => setIsCartOpen(true)} className="group flex h-11 px-6 items-center justify-center gap-3 rounded-full text-sm font-bold shadow-sm transition-all hover:shadow-md hover:translate-y-[-1px] bg-slate-900 text-white hover:bg-black">
              <StoreIcon name="cart" theme={theme} className="w-4 h-4 transition-transform group-hover:scale-110" />
              <span>Checkout {cartCount > 0 && <span className="ml-2 bg-white/20 rounded-full px-2 py-0.5 text-xs text-white">{cartCount}</span>}</span>
            </button>
          </div>
        </div>
      </div>
...
export function GamingPulseNav({ vendor, theme, cartCount, setIsCartOpen, handleShare, layoutWidthClass }: NavProps) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[100] transition-colors duration-500 text-white" style={{ backgroundColor: scrolled ? 'rgba(8,1,18,0.95)' : 'transparent', borderBottom: scrolled ? `1px solid rgba(139, 92, 246, 0.2)` : '1px solid transparent' }}>
      ...
      <div className={`mx-auto px-6 md:px-10 ${layoutWidthClass}`}>
        <div className={`items-center gap-6 ${theme.logo_position === 'center' ? 'grid grid-cols-3 min-h-[6rem] py-4' : 'flex justify-between h-16 md:h-20 transition-all text-white'}`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : (theme.logo_position === 'right' ? 'order-2' : '')}`}>
             {theme.logo_position === 'center' ? (
                <button onClick={handleShare} className="flex h-10 w-10 items-center justify-center group bg-white/5 rounded border border-white/10 hover:border-pink-500 transition-colors">
                  <StoreIcon name="share" theme={theme} className="w-5 h-5 text-indigo-300 group-hover:text-pink-400" />
                </button>
             ) : (
                <LogoBlock vendor={vendor} theme={theme} scrolled={scrolled} isTransparent />
             )}
          ...
          <div className={`flex items-center gap-4 justify-end`}>
            ...
            <button onClick={() => setIsCartOpen(true)} className="group relative flex h-11 px-5 items-center justify-center rounded-sm bg-white/5 border border-white/10 hover:border-pink-500 hover:bg-pink-500/10 transition-all font-bold tracking-widest text-[10px] uppercase">
              ...
              {cartCount > 0 && <span className="absolute -top-2 -right-2 flex h-5 min-w-[20px] items-center justify-center text-[9px] bg-pink-500 shadow-[0_0_10px_rgba(244,114,182,0.8)] text-white">{cartCount}</span>}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[100] transition-colors duration-500" style={{ backgroundColor: scrolled ? 'rgba(254, 250, 224, 0.95)' : theme.background_color, borderBottom: scrolled ? `1px solid ${theme.border_color}` : '1px solid transparent' }}>
      <div className={`mx-auto px-6 md:px-12 py-3 ${layoutWidthClass}`}>
        <div className={`flex items-center gap-6 py-2 ${theme.logo_position === 'center' ? 'grid grid-cols-3' : 'justify-between'}`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : (theme.logo_position === 'right' ? 'order-2' : '')}`}>
             {theme.logo_position === 'center' ? (
          ...
          <div className={`flex items-center gap-8 justify-end ${theme.logo_position === 'center' ? '' : ''}`} style={theme.logo_position === 'center' ? {} : { borderColor: theme.border_color }}>
            {theme.logo_position !== 'center' && (
               <button onClick={handleShare} className="text-xs uppercase tracking-[0.2em] transition-opacity hover:opacity-60" style={{ color: theme.text_color, fontFamily: FONT_MAP['dmSans'] }}>Discover</button>
            )}
  ...
  return (
    <header className="sticky top-0 z-[100]" style={{ backgroundColor: theme.background_color, borderBottom: `2px solid ${theme.border_color}` }}>
      <div className={`mx-auto px-4 sm:px-6 md:px-10 py-3 ${layoutWidthClass}`}>
        <div className={`items-center gap-4 ${theme.logo_position === 'center' ? 'grid grid-cols-3 min-h-[6rem] py-4' : 'flex justify-between h-12 md:h-16'}`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : (theme.logo_position === 'right' ? 'order-2' : '')}`}>
             {theme.logo_position === 'center' ? (
                <button onClick={handleShare} className="w-10 h-10 flex items-center justify-center bg-slate-50 rounded-full border shadow-sm transition-all hover:bg-slate-100" style={{ color: theme.primary_color }}>
                  <StoreIcon name="share" theme={theme} className="w-5 h-5" />
                </button>
             ) : (
                <LogoBlock vendor={vendor} theme={theme} />
          ...
          <div className="flex items-center gap-3 md:gap-5 justify-end">
            ...
            <button onClick={() => setIsCartOpen(true)} className="relative flex h-11 px-6 items-center justify-center gap-2 rounded-full font-bold uppercase tracking-widest text-xs shadow-md transition-all hover:translate-y-[-1px] hover:shadow-lg" style={{ background: theme.primary_gradient || `linear-gradient(45deg, ${theme.primary_color}, ${theme.accent_color})`, color: '#fff', fontFamily: FONT_MAP['montserrat'] }}>
              <StoreIcon name="cart" theme={theme} className="w-4 h-4 md:w-5 md:h-5" />
              <span className="hidden sm:inline">Checkout</span>
              {cartCount > 0 && <span className="absolute -top-1.5 -right-1.5 flex h-6 w-6 items-center justify-center rounded-full border-2 text-[10px] shadow-lg font-black" style={{ backgroundColor: '#fff', color: theme.primary_color, borderColor: theme.primary_color }}>{cartCount}</span>}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <header className="relative z-[100] mt-4 mb-2 mx-4" style={{ backgroundColor: 'transparent' }}>
      <div className={`mx-auto px-6 py-2 rounded-2xl md:rounded-[2rem] border transition-shadow hover:shadow-lg ${layoutWidthClass} bg-white/60 backdrop-blur-md`} style={{ borderColor: theme.border_color }}>
        <div className={`items-center gap-4 ${theme.logo_position === 'center' ? 'grid grid-cols-3 min-h-[5.5rem]' : 'flex h-14 md:h-16 justify-between'}`}>
          <div className={`${theme.logo_position === 'center' ? 'flex items-center justify-start' : (theme.logo_position === 'right' ? 'order-2' : '')}`}>
             {theme.logo_position === 'center' ? (
                <button onClick={handleShare} className="flex h-10 px-4 items-center justify-center rounded-xl bg-green-50/50 text-green-800 text-[10px] font-bold uppercase tracking-widest hover:bg-green-100 transition-colors" style={{ fontFamily: FONT_MAP['dmSans'] }}>Spread</button>
             ) : (
                <LogoBlock vendor={vendor} theme={theme} />
             )}
          </div>
          ...
          <div className="flex items-center justify-end gap-2 md:gap-4">
            ...
            <button onClick={() => setIsCartOpen(true)} className="flex h-11 px-5 items-center justify-center gap-2 rounded-xl text-sm font-bold transition-all shadow-sm hover:shadow-md border" style={{ backgroundColor: theme.surface_color, color: theme.primary_color, borderColor: theme.border_color }}>
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[100] transition-all duration-700" style={{ backgroundColor: scrolled ? theme.surface_color : 'transparent', borderBottom: scrolled ? `1px solid ${theme.border_color}` : '1px solid transparent', boxShadow: scrolled ? '0 10px 40px rgba(154, 52, 18, 0.05)' : 'none' }}>
      <div className={`mx-auto px-6 md:px-16 ${layoutWidthClass}`}>
        <div className={`items-center gap-6 ${theme.logo_position === 'center' ? 'grid grid-cols-3 min-h-[7rem] py-6' : 'flex justify-between h-20 md:h-24'}`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : (theme.logo_position === 'right' ? 'order-2' : 'flex-1')}`}>
             {theme.logo_position === 'center' ? (
                <button onClick={handleShare} className="flex h-11 w-11 items-center justify-center rounded-full border border-[rgba(251,191,36,0.1)] hover:bg-orange-50 transition-colors" style={{ color: theme.text_color }}>
                  <StoreIcon name="share" theme={theme} className="w-5 h-5" />
                </button>
             ) : (
                <LogoBlock vendor={vendor} theme={theme} scrolled={scrolled} />
          ...
          <div className={`flex items-center gap-4 justify-end ${theme.logo_position === 'center' ? '' : 'flex-1'}`}>
             {theme.logo_position !== 'center' && (
                <button onClick={handleShare} className="flex h-11 w-11 items-center justify-center rounded-full border border-[rgba(251,191,36,0.1)] hover:bg-orange-50 transition-colors" style={{ color: theme.text_color }}>
                  <StoreIcon name="share" theme={theme} className="w-4 h-4" />
                </button>
             )}
             <button onClick={() => setIsCartOpen(true)} className="group flex flex-col items-center gap-1 transition-all hover:opacity-70" style={{ color: theme.text_color }}>
                <StoreIcon name="cart" theme={theme} className="w-6 h-6 transition-transform group-hover:scale-110" />
                <div className="text-[10px] uppercase tracking-widest font-black flex items-center gap-1" style={{ fontFamily: FONT_MAP['inter'] }}>
                  Basket <span className="w-1.5 h-1.5 rounded-full bg-[#c2410c] inline-block"></span> {cartCount}
                </div>
             </button>
          </div>
        </div>
  ...
  return (
    <header className="relative z-[100] bg-[#fffbeb]">
      ...
      <div className={`mx-auto px-6 py-4 ${layoutWidthClass}`}>
        <div className={`items-center gap-6 ${theme.logo_position === 'center' ? 'grid grid-cols-3' : 'flex flex-col md:flex-row justify-between'} transition-all`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : (theme.logo_position === 'right' ? 'order-2' : '')}`}>
             {theme.logo_position === 'center' ? (
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[100] bg-white mix-blend-difference text-white">
      <div className={`mx-auto px-8 lg:px-16 ${layoutWidthClass}`}>
        <div className={`items-center gap-8 ${theme.logo_position === 'center' ? 'grid grid-cols-3' : 'flex justify-between'} h-20 md:h-24 transition-all uppercase`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : (theme.logo_position === 'right' ? 'order-2' : '')}`}>
             {theme.logo_position === 'center' ? (
                <button onClick={handleShare} className="text-xs font-medium tracking-[0.2em] hover:underline underline-offset-4">Share</button>
             ) : (
                <div className="flex items-center gap-12">
                   {theme.logo_url ? <img src={theme.logo_url} alt="" className="h-8 md:h-10 object-contain invert" /> : <h1 className="text-xl md:text-2xl font-bold tracking-tight" style={{ fontFamily: FONT_MAP['inter'] }}>{vendor.store_name || vendor.name}</h1>}
                </div>
             )}
          </div>
          ...
          <div className={`flex items-center gap-8 justify-end`}>
            ...
            <button onClick={() => setIsCartOpen(true)} className="flex items-center gap-2 text-xs font-medium tracking-[0.2em] hover:underline underline-offset-4">
              Bag {cartCount > 0 && <span className="opacity-50">[{cartCount}]</span>}
            </button>
          </div>
        </div>
 Broadway
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[100] transition-all duration-700" style={{ backgroundColor: scrolled ? 'rgba(2, 6, 23, 0.85)' : 'transparent', backdropFilter: scrolled ? 'blur(20px)' : 'none', borderBottom: scrolled ? `1px solid rgba(251, 191, 36, 0.2)` : '1px solid transparent' }}>
      <div className={`mx-auto px-6 lg:px-12 py-2 ${layoutWidthClass}`}>
        <div className={`items-center gap-6 ${theme.logo_position === 'center' ? 'grid grid-cols-3 min-h-[6.5rem]' : 'flex justify-between h-16 md:h-20'} transition-all`}>
          <div className={`${theme.logo_position === 'center' ? 'flex justify-start' : (theme.logo_position === 'right' ? 'order-2' : '')}`}>
             {theme.logo_position === 'center' ? (
                <button onClick={handleShare} className="flex h-11 w-11 items-center justify-center rounded-full border border-[rgba(251,191,36,0.3)] hover:bg-[rgba(251,191,36,0.1)] transition-all" style={{ color: theme.primary_color }}>
                   <StoreIcon name="share" theme={theme} className="w-5 h-5" />
                </button>
             ) : (
          ...
          <div className={`${theme.logo_position === 'center' ? 'flex justify-center text-center scale-110 md:scale-125 transition-transform' : 'flex-1 flex justify-center text-center'}`}>
