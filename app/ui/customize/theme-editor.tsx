...
export default function ThemeEditor({
  ...
  const draftData = useMemo(() => {
    ...
    try {
      return typeof theme.draft_config === "string"
        ? JSON.parse(theme.draft_config)
        : theme.draft_config;
    } catch (e) {
  ...
  const [sectionContent, setSectionContent] = useState<TemplateSectionContent>(
    () =>
      safeParse(
        draftData?.section_content ?? theme.section_content,
        getDefaultSectionContent(),
      ),
  );
  ...
  const hasUnsavedChanges = useMemo(() => {
    ...
    const themeChanged = keys.some(
      (k) => String(localTheme[k] ?? "") !== String(base[k] ?? ""),
    );
    const sectionsChanged =
      JSON.stringify(sections) !==
      (typeof baseSections === "string"
        ? baseSections
